...
test.describe("Settings Modal Layering Diagnostic", () => {
  test("should diagnose modal z-index and positioning", async ({ page }) => {
    ...
    if (headerRect && panelRect) {
      const overlap = {
        horizontal:
          panelRect.x < headerRect.x + headerRect.width &&
          panelRect.x + panelRect.width > headerRect.x,
        vertical:
          panelRect.y < headerRect.y + headerRect.height &&
      ...
      if (overlap.horizontal && overlap.vertical) {
        ...
        console.log(
          `Header bottom: ${overlap.headerBottom}, Panel top: ${overlap.panelTop}`,
        );

        if (overlap.panelTop < overlap.headerBottom) {
          console.log(
            "❌ Panel is positioned above header bottom - this is the issue!",
          );
        }
      } else {
    ...
    try {
      ...
      console.log(
        "❌ Settings button is not clickable - likely covered by modal",
      );
