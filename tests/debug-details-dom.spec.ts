  ...
  if (buttonCount > 0) {
    ...
    const buttonInfo = await page.evaluate(() => {
      const button = document.querySelector(
        'button:has-text("Details"), button span:has-text("Details")',
      ) as HTMLElement;
      ...
      const elementsWithStyle = Array.from(
        document.querySelectorAll('[style*="max-height"]'),
      );
    ...
    const afterClickInfo = await page.evaluate(() => {
      ...
      if ((window as Record<string, unknown>).domObserver) {
        (
          (window as Record<string, unknown>).domObserver as MutationObserver
        ).disconnect();
      }
      ...
      return {
        ...
        errorDetailsVisible: errorDetails
          ? !errorDetails.classList.contains("hidden")
          : false,
        errorDetailsDisplay: errorDetails
          ? window.getComputedStyle(errorDetails).display
          : "none",
        errorDetailsOpacity: errorDetails
          ? window.getComputedStyle(errorDetails).opacity
          : "0",
        errorDetailsMaxHeight: errorDetails
          ? window.getComputedStyle(errorDetails).maxHeight
          : "0px",
        errorDetailsClasses: errorDetails ? errorDetails.className : "",
        errorDetailsStyle: errorDetails
          ? errorDetails.getAttribute("style")
          : "",
      };
    });
    ...
    console.log(
      ...
      JSON.stringify(afterClickInfo, null, 2),
    );
