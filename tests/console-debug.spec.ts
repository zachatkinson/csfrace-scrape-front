  ...
  test('capture all console logs and page behavior', async ({ page }) => {
    ...
    await test.step('Check for details elements on page', async () => {
      ...
      if (detailsElements > 0) {
        for (let i = 0; i < detailsElements; i++) {
          ...
          console.log(`Details ${i + 1}: visible=${isVisible}, open=${isOpen}, summary="${summaryText}"`);
        }
      }
    });
        ...
        if (await details.isVisible()) {
          ...
          for (let j = 0; j < 5; j++) {
            ...
            if (currentState !== afterClickState) {
              console.log(`⚠️  Details element auto-changed state at check ${j + 1}: ${afterClickState} → ${currentState}`);
              break;
            }
          }
        }
    ...
    await test.step('Check DOM state and React components', async () => {
      ...
      const domInfo = await page.evaluate(() => {
        return {
          hasReactRoot: !!document.querySelector('#root, [data-reactroot], .App'),
          hasErrorBoundary: !!document.querySelector('.error-boundary, [data-testid="error-boundary"]'),
          scriptsCount: document.querySelectorAll('script').length,
          hasInlineScripts: Array.from(document.querySelectorAll('script')).some(s => !s.src),
