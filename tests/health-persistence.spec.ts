...
test.describe('Health Status Persistence', () => {
  test('should maintain health status data between page navigations', async ({ page }) => {
    ...
    for (let i = 0; i < healthStates.length; i++) {
      ...
      const currentServiceName = await postNavIndicators[i].getAttribute('data-service-name');
      const currentStatus = await postNavIndicators[i].getAttribute('data-status');
      ...
      console.log(`Service ${originalState.serviceName}: ${originalState.status} -> ${currentStatus} (persisted: ${originalState.status === currentStatus})`);
    }

    // Verify timestamp persists (should be the same unless a new poll happened)
    const timestampAfter = await page.locator('#health-timestamp').textContent();
    ...
    const healthPageIndicators = await page.locator('[data-service-name]').all();
    expect(healthPageIndicators.length).toBe(4);

    // Verify states are still maintained
    for (let i = 0; i < healthStates.length; i++) {
      const originalState = healthStates[i];
      const currentServiceName = await healthPageIndicators[i].getAttribute('data-service-name');
      const currentStatus = await healthPageIndicators[i].getAttribute('data-status');
      ...
      console.log(`Health page - Service ${originalState.serviceName}: status is ${currentStatus}`);
    }

    console.log('✅ Health status persistence test completed successfully');
  });

  test('should show loading states initially and update via polling', async ({ page }) => {
    ...
    const initialBackendStatus = await backendIndicator.getAttribute('data-status');
    ...
    const updatedBackendStatus = await backendIndicator.getAttribute('data-status');
