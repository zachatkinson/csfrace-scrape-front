...
test.describe('Health Monitoring System End-to-End Test', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('console', (msg) => {
      ...
      if (messageText.toLowerCase().includes('sse') ||
      ...
      if (messageText.toLowerCase().includes('nano') ||
  ...
  test('Complete Health Monitoring System Verification', async ({ page }) => {
    ...
    if (!healthElementFound) {
      console.log('No specific health status selectors found, proceeding with footer analysis');
    }
    ...
    for (let i = 0; i < 12; i++) {
      ...
      if (i % 3 === 0) {
        ...
        const footerText = await page.textContent('footer').catch(() => 'Footer not found');
        if (footerText && footerText.includes('Health')) {
          console.log('Footer contains health information: ' + footerText.slice(0, 100) + '...');
        }

        // Look for timestamp updates
        const timestampElements = await page.$$eval('*', (elements) => {
          return elements
            .filter(el => el.textContent && /\d{2}:\d{2}:\d{2}/.test(el.textContent))
            .map(el => el.textContent);
        });

        if (timestampElements.length > 0) {
          console.log('Found timestamp elements: ' + timestampElements.slice(0, 3).join(', '));
        }
      }
    }
    ...
    const hasGreenElements = footerHtml.includes('green') ||
    ...
    const hasStatusClasses = footerHtml.includes('status') ||
    ...
    // Success indicators
    const hasHealthIndicators = hasGreenElements || hasStatusClasses ||
