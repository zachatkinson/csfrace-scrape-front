...
test.describe('Details Button Fix', () => {
  test('details button should work properly without auto-closing', async ({ page }) => {
    ...
    if (await detailsButton.count() > 0) {
      ...
      console.log(`Details visibility after second click (should be closed): ${closedVisibility}`);
  ...
  test('details button rapid clicking test', async ({ page }) => {
    ...
    if (await detailsButton.count() > 0) {
      console.log('📱 Testing rapid clicking (should prevent race conditions)...');
      ...
      console.log(`Final state after rapid clicking: ${finalState ? 'visible' : 'hidden'}`);
