...
export interface LiquidInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'size'> {
  // Label and help text
  label?: string;
  helperText?: string;
  errorText?: string;
...
export const LiquidInput = forwardRef<HTMLInputElement, LiquidInputProps>(({
  ...
  const getSizeClasses = (): { container: string; input: string; icon: string } => {
    const sizes = {
      'sm': {
        container: 'h-9',
  ...
  const getStateClasses = (): string => {
    ...
    if (error) {
      classes.push('border-red-500/50 focus:border-red-500 focus:ring-red-500/20');
    } else if (success) {
      classes.push('border-green-500/50 focus:border-green-500 focus:ring-green-500/20');
    } else {
      classes.push('border-white/20 focus:border-blue-500/50 focus:ring-blue-500/20');
    }
    ...
    <div className={`relative ${className}`.trim()}>
      ...
      <div className="relative">
        ...
        >
          {/* Left Icon */}
          {leftIcon && (
            <div className={`flex-shrink-0 text-white/60 ml-3 ${sizeClasses.icon}`}>
              {leftIcon}
            </div>
          )}
          ...
          <div className="flex items-center gap-2 mr-3">
            ...
            {/* Right Icon */}
            {rightIcon && !loading && (
              <div className={`flex-shrink-0 text-white/60 ${sizeClasses.icon}`}>
                {rightIcon}
              </div>
            )}
          </div>
