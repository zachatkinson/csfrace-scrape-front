...
export class FilterUtils {
   ...
   */
  static extractAvailableStatuses(
  ...
  static filterJobs(
    ...
    if (filterState.currentFilter !== "all") {
      filteredJobs = filteredJobs.filter(
        (job) => job.status === filterState.currentFilter,
      );
    }
...
export class ValidationUtils {
  ...
  static validateFilterState(state: Partial<IFilterState>): IFilterState {
    ...
    return {
      ...
      selectedJobs:
        state.selectedJobs instanceof Set
          ? state.selectedJobs
          : defaultState.selectedJobs,

      totalJobs:
        typeof state.totalJobs === "number" && state.totalJobs >= 0
          ? state.totalJobs
  ...
  static validateJobsData(jobs: unknown[]): IJobData[] {
    ...
    return validJobs.map((job) => {
      ...
      const result: IJobData = {
        ...
        status: (job as { status: string }).status as
          | "pending"
          | "processing"
          | "completed"
          | "failed"
          | "queued",
        title:
          typeof (job as { title?: unknown }).title === "string"
            ? (job as { title: string }).title
            : "Untitled Job",
        url:
          typeof (job as { source_url?: unknown }).source_url === "string"
            ? (job as { source_url: string }).source_url
            : "Unknown URL",
        createdAt:
          (job as { createdAt?: unknown }).createdAt instanceof Date
            ? (job as { createdAt: Date }).createdAt
            : new Date(),
        updatedAt:
          (job as { updatedAt?: unknown }).updatedAt instanceof Date
            ? (job as { updatedAt: Date }).updatedAt
            : new Date(),
