...
class FilterCoordinator {
  ...
  private loadInitialState(): void {
    ...
    const stateFromDOM = {
      ...
      totalJobs: parseInt(
        domUtils.getDataAttribute(panel, "total-jobs") || "0",
        10,
      ),
    };
  ...
  private handleBatchAction(event: CustomEvent<IBatchActionEvent>): void {
    ...
    logger.info("Batch action processed", {
      action,
  ...
  private handleJobsDataUpdate(event: CustomEvent<IJobsDataUpdateEvent>): void {
    ...
    window.dispatchEvent(
      new CustomEvent("filterManager:updateStatuses", {
    ...
    window.dispatchEvent(
      new CustomEvent("batchManager:updateTotal", {
  ...
  private handleJobSelectionToggle(event: CustomEvent): void {
    ...
    window.dispatchEvent(
      new CustomEvent("batchManager:updateSelection", {
  ...
  private persistStateToDom(): void {
    ...
    domUtils.setDataAttribute(
      panel,
      "current-filter",
      this.currentState.currentFilter,
    );
    domUtils.setDataAttribute(
      panel,
      "current-sort",
      this.currentState.currentSort,
    );
    domUtils.setDataAttribute(
      panel,
      "search-query",
    ...
    domUtils.setDataAttribute(
      panel,
      "total-jobs",
      this.currentState.totalJobs.toString(),
    );
    domUtils.setDataAttribute(
      panel,
      "selected-jobs",
      this.currentState.selectedJobs.size.toString(),
    );
  }
  ...
  private exposePublicInterface(): void {
    ...
    (
      window as Window & { filterPanelCoordinator?: IFilterPanelCoordinator }
    ).filterPanelCoordinator = {
