...
import { StandardizedSettingsPanel } from '../settings/StandardizedSettingsPanel';
                  ...
                  <div className="border-t border-white/10 pt-4">
                    ...
                    <div className="flex gap-2 justify-center">
                      {oauthProviders.map((provider: any) => (
                        <button
                          key={provider.name}
                          onClick={() => handleOAuthLogin((provider as any).name)}
                          className={`glass-button px-4 py-2 flex items-center gap-2 text-sm font-medium transition-colors
                            ${(provider as any).name === 'google' ? 'text-red-300 hover:text-red-200 hover:bg-red-500/20' : ''}
                            ${(provider as any).name === 'github' ? 'text-gray-300 hover:text-gray-200 hover:bg-gray-500/20' : ''}
                            ${(provider as any).name === 'microsoft' ? 'text-blue-300 hover:text-blue-200 hover:bg-blue-500/20' : ''}
                          `}
                        >
                          ...
                          {(provider as any).name === 'microsoft' && (
                            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                              <path d="M0 0h11v11H0V0zm13 0h11v11H13V0zM0 13h11v11H0V13zm13 0h11v11H13V13z"/>
                            </svg>
                          )}
                          <span className="capitalize">{(provider as any).display_name || (provider as any).name}</span>
                        </button>
                      ))}
                    </div>
                  </div>
