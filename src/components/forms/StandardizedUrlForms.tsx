...
function renderSingleUrlFields(
  ...
  options: { showOptions?: boolean; isAuthenticated: boolean } = {
    showOptions: false,
    ...
    <>
          ...
          leftIcon={
            ...
            >
              <path
                ...
                strokeWidth={2}
                d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.102m0 0l4-4a4 4 0 105.656-5.656l-4 4m-4 4l4-4m0 0l-4-4m4 4l4-4"
              />
            </svg>
          }
          required
      ...
      {options.showOptions && (
        <FormSection
          ...
          subtitle="Customize how your content is converted"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center space-x-2 text-sm text-white/80">
              <input
                ...
                onChange={(e) =>
                  handlers.handleFieldChange("options")({
                    ...(data.options || {}),
...
export const StandardizedSingleUrlForm: React.FC<
  StandardizedSingleUrlFormProps
> = ({
  onSubmit,
  onSuccess,
  onError,
  ...
  return (
    <div className={className}>
      <BaseForm
        ...
        validationSchema={
          singleUrlValidationSchema as FormValidationSchema<unknown>
        }
        onSubmit={
          handleSubmit as (
            data: unknown,
        ...
        renderFields={(formHook) =>
          renderSingleUrlFields(
            formHook as ReturnType<typeof useBaseForm<SingleUrlFormData>>,
            {
              showOptions,
              isAuthenticated,
            },
  ...
  return (
    <>
      ...
      <FormField error={state.errors.batchUrls}>
        <div className="space-y-2">
          ...
          <textarea
            ...
            onChange={(e) =>
              handlers.handleFieldChange("batchUrls")(e.target.value)
            }
          ...
          <p className="text-xs text-white/60">
            Enter up to {options.maxUrls} URLs, one per line. You can also
            upload a file below.
          </p>
        </div>
      </FormField>
      ...
      <FormSection
        ...
        subtitle="Upload a TXT, CSV, or JSON file with URLs"
      >
        <div className="space-y-4">
          ...
          <p className="text-xs text-white/50">
            Supported formats: TXT (one URL per line), CSV (URL column), JSON
            (array of URLs)
          </p>
        </div>
      </FormSection>
      ...
      {options.showOptions && (
        <FormSection
          title="Batch Options"
          subtitle="Apply these settings to all URLs"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center space-x-2 text-sm text-white/80">
              <input
...
export const StandardizedBatchUrlForm: React.FC<
  StandardizedBatchUrlFormProps
> = ({
  onSubmit,
  onSuccess,
  onError,
  ...
  const handleSubmit = async (
    data: BatchUrlFormData,
  ): Promise<FormSubmissionResult> => {
    if (!onSubmit) {
      return { success: true, data };
    }

    const urls = data.batchUrls
      .split("\n")
      .map((u) => u.trim())
      .filter(Boolean);
  ...
  return (
    <div className={className}>
      <BaseForm
        ...
        validationSchema={
          batchUrlValidationSchema as FormValidationSchema<unknown>
        }
        onSubmit={
          handleSubmit as (
            data: unknown,
        ...
        className="w-full"
        renderFields={(formHook) =>
          renderBatchUrlFields(
            formHook as ReturnType<typeof useBaseForm<BatchUrlFormData>>,
            fileInputRef,
            {
              showOptions,
              isAuthenticated,
