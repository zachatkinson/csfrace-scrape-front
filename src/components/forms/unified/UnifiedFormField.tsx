...
const FieldErrorDisplay: React.FC<{
  ...
  return (
    <div
      className={`mt-1 p-2 text-sm rounded-glass border ${colors[type]} ${config.className || ""}`}
    >
 ...
 */
const FieldHelpText: React.FC<{ text: string; className?: string }> =
...
const RadioGroup: React.FC<{
  ...
  ({ name, options, value, onChange, disabled, className = "" }) => (
...
export const UnifiedFormField: React.FC<IUnifiedFieldProps> = React.memo(
  ({
    ...
    const renderFieldInput = () => {
      switch (type) {
        ...
        case "checkbox":
          return (
            <label
              className={`flex items-center space-x-3 cursor-pointer group ${className}`.trim()}
            >
              <input
                {...commonProps}
                type="checkbox"
    ...
    return (
      <div
        className={`unified-form-field ${fullWidth ? "w-full" : ""} ${className}`.trim()}
      >
        {renderFieldInput()}

        {/* Help Text */}
        {helpText && <FieldHelpText text={helpText} />}
