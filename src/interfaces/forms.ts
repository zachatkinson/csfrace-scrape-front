...
export interface FormDataProps<TData = unknown> {
  onSubmit?:
    | ((data: TData) => Promise<FormSubmissionResult> | FormSubmissionResult)
    | undefined;
}
...
export interface FormComponentProps<TData = unknown>
  ...
  children?:
    | ReactNode
    | ((formHook: FormHookReturn<TData>) => ReactNode)
    | undefined;
}
 ...
 */
export type StandardFormComponent<TData = unknown> = React.FC<
