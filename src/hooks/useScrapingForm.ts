...
export function useScrapingForm(options: UseScrapingFormOptions = {}) {
  ...
  // Services
  const [urlValidationService] = useState(
    () =>
      new UrlValidationService({
        onValidationStart: () =>
          setState((prev) => ({ ...prev, isValidating: true })),
        onValidationComplete: (_url: string, result: UrlValidationResult) =>
          setState((prev) => ({
            ...prev,
            validationResult: result,
            isValidating: false,
          })),
        onValidationError: (_url: string, error: string) =>
          setState((prev) => ({
            ...prev,
            error,
            isValidating: false,
  ...
  const [fileProcessingService] = useState(
    () =>
  ...
  const isValidForSubmission = useCallback((): boolean => {
    if (state.batchMode) {
      ...
      return (
        urls.length > 0 &&
        urls.length <= maxBatchUrls &&
        !state.isBatchSubmitting
      );
    } else {
      return Boolean(
  ...
  const validateCurrentForm = useCallback(async () => {
    if (state.batchMode) {
      ...
      if (urls.length > maxBatchUrls) {
        return {
          isValid: false,
          errors: [`Too many URLs. Maximum is ${maxBatchUrls}`],
        };
      }
  ...
  return {
    ...
    computed: {
      isLoading:
        state.isValidating || state.isSubmitting || state.isBatchSubmitting,
