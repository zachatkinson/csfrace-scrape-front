...
export class HealthUIHelper {
  static createGlassButton(type: "copy" | "clear", id: string, title: string) {
    ...
    button.title = title;
    button.className =
      "glass-button px-3 py-1 text-xs text-white/80 hover:text-white border border-white/20 hover:border-white/40 rounded transition-all duration-200";
  ...
  static updateAllLatencyFromHealthData(
    healthData: Record<string, unknown>,
    apiLatency: number,
  ) {
    this.updateLatencyElement(
      "api-latency",
      `${apiLatency}ms`,
      apiLatency,
      "API",
