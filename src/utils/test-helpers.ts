...
export class SecurityTestHelpers {
  ...
  static async testXSSProtection(page: Page, inputSelector: string): Promise<void> {
  ...
  static async testCSRFProtection(page: Page, formSelector: string): Promise<void> {
    await test.step('Test CSRF protection', async () => {
      ...
      await page.evaluate((selector) => {
        ...
        const csrfInput = form.querySelector('input[name="csrf-token"], input[name="_token"]') as HTMLInputElement;
        if (csrfInput) {
          csrfInput.remove();
        }
      }, formSelector);
  ...
  static async runFullSecurityAudit(page: Page, options: ISecurityTestOptions = {}): Promise<void> {
    const {
      checkCSP = true,
...
export class AuthTestHelpers {
  ...
  static async setupAuthenticatedSession(context: BrowserContext, user: ITestAuthUser): Promise<void> {
...
export class PerformanceTestHelpers {
  ...
  static async measureFirstContentfulPaint(page: Page, url: string): Promise<number> {
    await page.goto(url);
  ...
  static async auditBundleSize(page: Page): Promise<{ totalSize: number; jsSize: number; cssSize: number }> {
    await page.goto('/');
      ...
      const contrastIssues = await page.evaluate(() => {
        ...
        elements.forEach((element) => {
          ...
          if (color === backgroundColor) {
            issues.push(`Element has same color and background-color: ${element.tagName}`);
          }
        });
