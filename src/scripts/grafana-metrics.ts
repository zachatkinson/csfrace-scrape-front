...
class GrafanaMetricsUpdater {
  ...
  async updateGrafanaMetrics(): Promise<void> {
    ...
    try {
      ...
      this.logger.info("Grafana dashboard metrics updated successfully", {
        metrics,
      });
  ...
  private updatePluginInfo(metrics: GrafanaMetrics): void {
    ...
    const monitoringPanelElement = document.getElementById(
      "grafana-monitoring-panel",
    );
    if (monitoringPanelElement) {
      const { icon, text, className } = this.getPluginDisplay(
        metrics.monitoringPanelStatus,
      );
    ...
    if (pluginContainer && metrics.plugins.length > 1) {
      ...
      pluginContainer.innerHTML = additionalPlugins
        .map(
          (plugin) =>
            `<div class="text-xs text-green-400">✅ ${this.formatPluginName(plugin)}</div>`,
        )
        .join("");
    }
  }
  ...
  private getPluginDisplay(pluginStatus: string): {
    icon: string;
    text: string;
    className: string;
  } {
...
if (typeof window !== "undefined") {
  ...
  if (document.readyState === "complete") {
    moduleLogger.info(
      "Document ready - updating Grafana dashboard metrics immediately",
    );
  ...
  setInterval(() => {
    moduleLogger.info(
      "Periodic Grafana dashboard metrics update (1-minute poll)",
    );
    updater.updateGrafanaMetrics();
  }, 60000);
}
