...
class GreetingService implements IGreetingService {
  ...
  private readonly fallbackGreeting =
    "Ready to convert something amazing today?";
  ...
  getCurrentGreeting(): string {
    try {
      ...
      this.logger.error(
        "Failed to get current greeting, using fallback",
        error as Error,
      );
  ...
  getRandomGreeting(period: string): string {
    ...
    if (!messages || !Array.isArray(messages) || messages.length === 0) {
      this.logger.warn("No messages found for period, using fallback", {
        period,
      });
      return this.fallbackGreeting;
    }
    ...
    if (!selectedMessage || typeof selectedMessage !== "string") {
      this.logger.warn("Invalid message selected, using fallback", {
        period,
        randomIndex,
      });
      return this.fallbackGreeting;
  ...
  addGreetingsForPeriod(period: string, messages: string[]): void {
    ...
    this.logger.info("Added new greetings for period", {
      period,
