    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (
          ...
          // Exponential backoff
          const delay = TIMING_CONSTANTS.HELPERS.exponentialBackoff(
...
export const api = {
  ...
  submitJob: (url: string, options: ConversionOptions) =>
    apiClient.submitJob(url, options),
  ...
  downloadJobResult: (jobId: string, format?: "html" | "json") =>
    apiClient.downloadJobResult(jobId, format),
  submitBatch: (urls: string[], options: ConversionOptions, name?: string) =>
    apiClient.submitBatch(urls, options, name),
  getBatch: (batchId: string) => apiClient.getBatch(batchId),
  getBatches: () => apiClient.getBatches(),
  getStats: (period?: { start: string; end: string }) =>
    apiClient.getStats(period),
  uploadFile: (file: File, options: ConversionOptions) =>
