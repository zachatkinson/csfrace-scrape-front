...
class AuthAPI {
  ...
  async initiateOAuthLogin(request: OAuthLoginRequest): Promise<{ authorization_url: string }> {
    return this.baseClient['request']<{ authorization_url: string }>('/auth/oauth/login', {
      method: 'POST',
  ...
  async getPasskeySummary(): Promise<WebAuthnCredential[]> {
    return this.baseClient['request']<WebAuthnCredential[]>('/auth/passkeys/summary');
  }

  async beginPasskeyRegistration(name?: string): Promise<WebAuthnRegistrationResponse> {
    return this.baseClient['request']<WebAuthnRegistrationResponse>('/auth/passkeys/register/begin', {
      method: 'POST',
      body: JSON.stringify({ name }),
    });
  }

  async completePasskeyRegistration(data: WebAuthnVerificationRequest): Promise<WebAuthnCredential> {
    return this.baseClient['request']<WebAuthnCredential>('/auth/passkeys/register/complete', {
      method: 'POST',
  ...
  async beginPasskeyAuthentication(): Promise<WebAuthnAuthenticationResponse> {
    return this.baseClient['request']<WebAuthnAuthenticationResponse>('/auth/passkeys/authenticate/begin', {
      method: 'POST',
    });
  }

  async completePasskeyAuthentication(data: WebAuthnVerificationRequest): Promise<AuthLoginResponse> {
    return this.baseClient['request']<AuthLoginResponse>('/auth/passkeys/authenticate/complete', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
...
export function shouldRefreshToken(expiresAt: number, bufferMinutes: number = 5): boolean {
